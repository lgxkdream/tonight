...
<div id="page-content" class="clearfix">
  <div class="row-fluid">
	<div class="row-fluid">
			<!-- 检索  -->
			<form action="homeworkTemplet/list.do" method="post" name="Form" id="Form">
			<table>
				<tr>
					<td>
					...
					<c:when test="${not empty varList}">
						...
						<c:forEach items="${varList}" var="var" varStatus="vs">
							<tr>
								...
								<td>${fn:substring(var.TITLE,0,20)}<c:if test="${fn:length(var.TITLE)>20}">...</c:if></td>
								<td>${var.TYPE_NAME}</td>
								<td style="width: 50px;">
								<c:choose>
								<c:when test="${var.HOMEWORK_IDS!=null&&var.HOMEWORK_IDS!=''}">
								<a href="javascript:top.siMenu('${var.ID}','${var.ID}','${var.TITLE}-模板内容列表','homeworkTemplet/listFromTemplet.do?TEMPLET_ID=${var.ID}')">
								${fn:length(fn:split(var.HOMEWORK_IDS,","))}
								</a>
								</c:when>
								...
								<td style="width: 30px;" class="center">
									...
									<c:if test="${QX.edit != 1 && QX.del != 1 }">
										...
										<c:if test="${QX.edit == 1 }">
										<button title="分配题目" class="btn btn-mini btn-info" data-toggle="dropdown" onclick="assignHomework('${var.ID}');">分配题目</button>
										<button title="编辑" class="btn btn-mini btn-info" data-toggle="dropdown" onclick="edit('${var.ID}');">编辑</button>
										</c:if>
										<c:if test="${QX.del == 1 }">
										<button title="删除" class="btn btn-mini btn-info" data-toggle="dropdown" onclick="del('${var.ID}');">删除</button>
										</c:if>
									</div>
								</td>
							</tr>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function assignHomework(Id){
			top.jzts();
			 ...
			 diag.Drag=true;
			 diag.Title ="分配题目";
			 diag.URL = '<%=basePath%>homeworkTemplet/beforeAssignHomework.do?TEMPLET_ID='+Id;
			 diag.Width = 950;
			 diag.Height = 650;
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>homeworkTemplet/goAdd.do';
			 diag.Width = 450;
			 diag.Height = 355;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>homeworkTemplet/goEdit.do?ID='+Id;
			 diag.Width = 450;
			 diag.Height = 355;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>homeworkTemplet/delete.do?ID="+Id+"&tm="+new Date().getTime();
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>homeworkTemplet/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
