...
<body>
						...
						<table>
							<tr>
								<td>
									<span class="input-icon"> 
										<input 
											...
											type="text" name="TITLE" value="${pd.TITLE }" placeholder="这里输入标题关键词" 
										/> 
								...
								<td style="vertical-align: top;">
									<select
										class="chzn-select" name="TYPE" id="TYPE"
										data-placeholder="请选择分类"
										style="vertical-align: top; width: 120px;">
										<option value="">全部分类</option>
										<c:forEach var="data" items="${dic_homework_type_list}" >
									    	<option value='${data.ID}' 
									    		<c:if test='${data.ID==pd.TYPE}'>selected</c:if>>
									    			${data.NAME}
									    	</option>
									    </c:forEach>
									</select>
	...
	<script type="text/javascript">
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>homework/goEdit.do?ID='+Id;
			 diag.Width = 600;
			 diag.Height = 400;
			 diag.CancelEvent = function(){ //关闭事件
				 //if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
